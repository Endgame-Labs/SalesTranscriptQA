...
def assess(candidate,calls,transport):
    ...
    for model in [PRIMARY,SECONDARY]:
        ...
        for repair in range(2):
            structured=isinstance(v,dict) and type(v.get('necessary')) is bool and isinstance(v.get('exclusive_facts'),list) and isinstance(v.get('reason'),str) and bool(v['reason'].strip())
            if structured:
                break
        ...
        passed=bool(valid and v.get('necessary') is True)
